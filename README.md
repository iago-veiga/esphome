...
`make update-all` consulta primero la versión real de todos los dispositivos
mediante su API nativa cifrada:

- Si coincide con la versión del contenedor, omite el dispositivo.
- Si es distinta, compila y actualiza por OTA.
...
La consulta usa `esphome_api_encryption_key` de `secrets.yaml`. El hostname se
obtiene de `substitutions.device_name`; si no existe, usa nombre del YAML.
...
- `scripts/device_version.py`: consulta versión por API; uso interno.
